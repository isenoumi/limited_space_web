<template>
  <el-upload v-model:file-list="fileList" class="upload-demo" :action="actionUrl" multiple :before-upload="beforeUpload" :on-success="handleSuccess" :on-remove="handleRemove" :limit="limit" :show-file-list="showFile" :data="data" :headers="headers" v-loading.fullscreen="loading">
  ...
  if (props.actionUrl) {
    ...
    if (props.planId && props.projectId) {
      actionUrl.value =
        ...
        `/yx-project/api/t_project_plan/importExcel`
      data.value = {
        planId: props.planId,

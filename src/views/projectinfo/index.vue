<template>
  <div class="app-container">
    <el-card>
      <el-tabs v-model="currentTab" class="demo-tabs" @tab-click="handleClickTab" v-hasPerm="['PROJECT_ALL']">
      ...
      <div class="header">
        <div class="headInput">
          ...
          <el-input v-model="query.projectName" placeholder="请输入项目名称" class="input"></el-input>
        </div>
        <div class="headInput">
          <div class="title">项目编号:</div>
          <el-input v-model="query.projectId" placeholder="请输入项目编号" class="input"></el-input>
        </div>
        <div class="headSelect">
          <div class="title">项目类型:</div>
          <el-select v-model="query.projectType" :multiple="true" placeholder="请选择项目类型" clearable>
            <el-option v-for="item in options.typeOptions" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </div>
        ...
        <div class="headSelect">
          ...
          <el-select v-model="query.projectStatus" placeholder="请选择项目状态" clearable>
            <el-option v-for="item in options.statusOptions" :key="item.value" :label="item.label" :value="item.value" :style="{ color: item.color }">
            </el-option>
          </el-select>
        </div>
        <div class="headSelect">
          <div class="title">项目跟进状态:</div>
          <el-select v-model="query.projectFollowStatus" placeholder="请选择项目跟进状态" clearable>
        ...
        <div class="headSelect">
          ...
          <el-cascader v-model="query.area" :options="cityData" :props="cityProps" ref="cascaderAddr" placeholder="请选择省市区" clearable></el-cascader>
        </div>
        <div class="headSelect" v-if="activeTab == 'first'">
          <div class="title">销售人员:</div>
          <el-select filterable v-model="query.salesperson" placeholder="请选择销售人员" clearable>
            <el-option v-for="item in options.salespersonOptions" :key="item.label" :label="item.label" :value="item.label">
            </el-option>
          </el-select>
        </div>
        <div class="headSelect">
          <div class="title">项目优先级:</div>
          <el-select v-model="query.projectFirstLevel" placeholder="请选择项目优先级" clearable>
            <el-option v-for="item in options.priorityOptions" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </div>
        <div class="btnList">
          <el-button type="primary" @click="queryList">查询</el-button>
          <el-button style="margin-right: 10px" type="primary" @click="reset">重置</el-button>
          <!-- <fileUpload
            :label="'导入项目'"
            action-url="/yx-project/api/project/projectImport"
      ...
      <div class="main">
        <el-table :data="tableData" border style="width: 100%" v-loading="loading">
          <el-table-column align="center" label="序号" type="index" width="60" :index="(e) => e + 1 + (query.pageNo - 1) * query.pageSize" />
          <el-table-column align="center" label="项目编号" prop="projectId" width="115" />
          <el-table-column align="center" label="所属公司" prop="company" />
          <el-table-column align="center" label="项目名称" prop="projectName" width="100">
          ...
          <el-table-column align="center" label="项目类型" prop="projectType">
            <template #default="scope">
              <span v-for="(item1, index) in options.typeOptions" :key="index">
                <span v-for="(item2, index2) in scope.row.projectType" :key="index2">
                  <span v-if="item2 == item1.value">{{ item1.label }} &nbsp;</span>
                </span>
              </span>
            </template>
          </el-table-column>
          <el-table-column align="center" label="项目资金来源" prop="projectSourceFunds">
            <template #default="scope">
              <div v-for="(item, index) in options.projectSourceFundsOptions" :key="index">
          ...
          <el-table-column align="center" label="所属区域" prop="area" width="140">
            <template #default="scope">
              <span>
                {{
          ...
          <el-table-column align="center" label="客户名称" prop="customerName" width="100">
            <template #default="scope">
              <el-tooltip class="box-item" effect="dark" :content="scope.row.customerName" placement="top-start">
                {{
          ...
          /> -->
          <el-table-column align="center" label="预合同金额（万）" prop="prepareContractAmount" />
          ...
          <el-table-column align="center" label="项目状态" prop="projectStatus">
            <template #default="scope">
              <div v-for="(item, index) in options.statusOptions" :key="index">
                <div v-if="item.value == scope.row.projectStatus" :style="{ color: item.color }">
                  {{ item.label }}
                </div>
              </div>
            </template>
          </el-table-column>
          <el-table-column align="center" label="项目跟进状态" prop="projectFollowStatus">
            <template #default="scope">
              <div :style="{
          ...
          <el-table-column align="center" label="项目进度" prop="projectSchedule" />
          <el-table-column align="center" label="项目优先级" prop="projectFirstLevel">
            <template #default="scope">
          ...
          <el-table-column align="center" label="操作" width="380">
            <template #default="scope">
              <el-button v-if="
                ...
                " v-hasPerm="['PROJECT_EDIT']" text type="primary" @click="handleEditProject(scope.row)">编辑
              </el-button>
              <el-button v-if="
                  scope.row.projectStatus == 'NOTSTARTED' ||
                  scope.row.projectStatus == 'BEGINPROJECT' ||
                  scope.row.projectStatus == 'INPROGRESS'
                " text type="primary" @click="handleVisit(scope.row)">客户跟进
              </el-button>
              <el-button v-if="
                  scope.row.projectStatus == 'NOTSTARTED' ||
                  scope.row.projectStatus == 'BEGINPROJECT'
                " text type="primary" @click="handlePreSalesSupport(scope.row)">售前支持</el-button>
              <el-button v-if="scope.row.projectStatus == 'NOTSTARTED'" text type="primary" @click="handleProjectMsg(scope.row)">立项
              </el-button>
              <el-button v-if="scope.row.projectStatus == 'BEGINPROJECT'" text type="primary" @click="handleBiddingMsg(scope.row)">招投标
              </el-button>
              <el-button v-if="scope.row.projectStatus" text type="primary" @click="handleMobilization(scope.row)">
                进场
              </el-button>
              <el-button v-if="scope.row.projectStatus" text type="primary" @click="handleContract(scope.row)">合同签订
              </el-button>

              <el-button v-if="
                  scope.row.projectStatus == 'FINISHED' ||
                  scope.row.projectStatus == 'NOFINISHED'
                " text type="primary" @click="handleInfo(scope.row)">详情
              </el-button>
              <!-- <el-button
        ...
        <div class="page">
          ...
          <pagination v-if="query.total > 0" v-model:total="query.total" v-model:page="query.pageNo" v-model:limit="query.pageSize" @pagination="queryList" />
        </div>
      </div>
    </el-card>
    <div v-if="addProjectVisible">
      <addProject :id="associateTheCustomerId" v-model="addProjectVisible" :formData="projectFormData" type="edit" @reset="reset"></addProject>
    </div>

    <div v-if="projectInfoVisible">
      <addVisit :id="visitId" :clientVisitId="associateTheCustomerId" :type="visitType" :projectFormData="projectInfo" v-model="projectInfoVisible" @reset="reset"></addVisit>
    </div>

    <div v-if="detailProjectVisible">
      <detailProject :id="clientVisitId" :clientVisitId="associateTheCustomerId" :projectFormData="detailProjectInfo" v-model="detailProjectVisible" @reset="reset"></detailProject>
    </div>
    <div v-if="biddingVisible">
      <biddingComponents :id="id" v-model="biddingVisible" @reset="reset"></biddingComponents>
    </div>
    <div v-if="mobilizationVisible">
      <Mobilization :id="id" v-model="mobilizationVisible" :projectName="ApprovalFormData" @reset="reset"></Mobilization>
    </div>
  </div>
</template>
<!-- 图标选择器示例 -->

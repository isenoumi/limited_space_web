  ...
  <el-card v-loading="imgLoading">
    <div class="searchBox">
      <div class="headerSelect">
        ...
        <div class="inputBox" v-if="isCounty">
          <div class="title">企业（单位）名称:</div>
          <el-input v-model="searchForm.enterpriseName" class="input" clearable placeholder="请输入企业（单位）名称">
          </el-input>
        </div>
        <div class="inputBox" v-if="isEnterprise">
    ...
    <div class="buttonControl">
      ...
      <!-- <el-button type="info" @click="reset">导入</el-button> -->
    ...
    <proTable v-loading="loading" :tableHeader="tableHeaderData" :isMoreSelect="false" :isShowIndex="true" :tableData="tableList" :currentChange="handleCurrentChange" :sizeChange="handleSizeChange" :pageSize="pageSizes" :currentPage="currentPages" :totalNum="total">
      <template v-slot:default="{ rows, cols }">
        ...
        <div v-if="cols.props == 'controls'">
          ...
          <el-button class="controlsButton" type="primary" @click="createQRCode(rows)" text v-if="isEnterprise"> 查看二维码 </el-button>
          <!-- <el-button
    ...
    <countyLedger :formValue="formValue" :type="clickType" v-model="countyLedgerVisible" :id="id" @reset="reset" :clickType="clickType"></countyLedger>
    ...
    <cityLedger :formValue="formValue" :type="clickType" v-model="cityLedgerVisible" @reset="reset" :clickType="clickType"></cityLedger>
    <!-- </div> -->
  </el-card>
</template>
<script setup>
  ...
  import countyLedger from "./components/countyLedger.vue"
  import enterpriseLedger from "./components/enterpriseLedger.vue"
  import cityLedger from "./components/cityLedger.vue"
  ...
  const view = (row) => {
    ...
    if (isCity.value) {
      router.push({
        path: "/viewEnterpriseLedger",
        query: { companyName: row.enterpriseName },
      })
    }
    formValue.value = JSON.parse(JSON.stringify(row))
  ...
  onMounted(() => {
    console.log("isCounty.value", isCounty.value)
    //初始化
    if (isEnterprise.value) {
      tableHeaderData.value = enterprise
    } else if (isCounty.value) {
      tableHeaderData.value = county
    } else if (isCity.value) {
      tableHeaderData.value = city
    }

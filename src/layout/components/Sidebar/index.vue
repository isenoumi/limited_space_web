<script setup lang="ts">
  ...
  onMounted(() => {
    ...
    //后端返回菜单处理
    menu.map((item: any) => {
      if (item.name.includes("泸州市城管领域有限空间安全管理系统")) {
...
<template>
  <div :class="{ 'has-logo': sidebarLogo }">
    ...
    <el-scrollbar>
      <el-menu :default-active="currRoute.path" :collapse="!appStore.sidebar.opened" :background-color="variables.menuBg" :text-color="variables.menuText" :active-text-color="variables.menuActiveText" :unique-opened="false" :collapse-transition="false" mode="vertical">
        <sidebar-item v-for="route in rowList.datalist" :key="route.path" :item="route" :base-path="route.path" :is-collapse="!appStore.sidebar.opened" />
      </el-menu>
    </el-scrollbar>
  </div>
</template>
